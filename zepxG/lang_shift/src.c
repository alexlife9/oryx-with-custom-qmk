...
bool lang_shift_process_custom_keycodes(Key key, keyrecord_t* record) {
  ...
  switch (key) {
    ...
    case GO_TO_RU:
      if (down) {
        // Шаг 1: Полностью сбрасываем все активные слои.
        // Клавиатура гарантированно окажется на слое 0.
        layer_clear(); 
    ...
    case GO_TO_EN:
       if (down) {                  // Действуем только при нажатии
        lang_activate_from_user(1); // 1. Устанавливаем целевой язык - английский (1)
        layer_on(1);              // 2. Принудительно переключаем клавиатуру на слой 1
      }
      return false;   
    case AG_3DOT:
